...
def filter_binary_file(dp, filt_key='highpass', suffix='_processed', verbose=False, **kwargs):
    ...
    # Memory map the contents of our original file
    # This allows us to use its dimensions (which have already been validated by mmap_binary_file)
    original_voltage = mmap_binary_file(dp, filt_key=filt_key, prefer_processed=False)
    
    original_fp = open(original_binary_file_path, 'rb')
    processed_fp = open(processed_binary_file_path, 'wb')
    ...
    for j in range(0, original_voltage.shape[1]):
        # Read num_channels int16's
        voltage_across_channels = np.fromfile(original_fp, dtype=original_voltage.dtype, count=original_voltage.shape[0])
        assert len(voltage_across_channels) == original_voltage.shape[0]
        for i in range(0, len(voltage_across_channels)):
            voltage_across_channels[i] = filters[i].filter(voltage_across_channels[i]).astype(original_voltage.dtype)
        processed_fp.write(voltage_across_channels.tobytes())
        if verbose and (j % (original_voltage.shape[1] // 10)) == 0:
